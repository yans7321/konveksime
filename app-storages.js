...
(function () {
  ...
  function renderHistory(items) {
    ...
    if (!tbody) return;
    var startEl = el("t3-server-history-start");
    var endEl = el("t3-server-history-end");
    ...
    items.forEach(function (s) {
      if (useRange && !inRange(s.storedAt, startIso, endIso)) return;
      var tr = document.createElement("tr");
      tr.className = "hover:bg-slate-50 transition border-b border-slate-100";
      tr.innerHTML =
        '<td class="p-3 text-slate-500">' + fmtDate(s.storedAt) + "</td>" +
